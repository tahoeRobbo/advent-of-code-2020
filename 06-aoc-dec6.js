...
const solvePart1 = (groupAnswers) => {
  let count = 0;

  groupAnswers.forEach((answers) => {
    answers = answers.replace(/\s/g, '')
    count += new Set(answers).size;
  })
...
const solvePart2 = (groupAnswers) => {
  let count = 0;

  groupAnswers.forEach((answers) => {
    let allAnswers = answers.replace(/\s/g, '')
    let individualAnswers = answers.split('\n')

    let uniqueAnswers = Array.from(new Set(allAnswers));

    for (let j = 0, len = uniqueAnswers.length; j < len ; j++) {
      if (individualAnswers.every((answers) => answers.includes(uniqueAnswers[j]))) {
        count++;
      }
    }
  })
