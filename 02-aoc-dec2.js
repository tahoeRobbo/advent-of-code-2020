...
const passwordList = getPasswordList();
const solvePart1 = (passwordList) => {

  return passwordList.filter((passwordData) => {
    let rule;
    let character;
    let password;
    ...
    [rule, character, password] = passwordData.split(' ');
    [min, max] = rule.split('-');

    const strippedPassword = password.replace( new RegExp(`[^${character[0]}]`, 'gi'), '')
...
let answerPart1 = solvePart1(passwordList)
console.log('answer1', answerPart1)
...
let answerPart2 = solvePart2(passwordList);
console.log('answerPart2', answerPart2);

function getPasswordList () {
  const passwordListString = `9-12 q: qqqxhnhdmqqqqjz
...
9-11 b: bkbltdvbtwbbtsb`
  return passwordListString.split('\n');
}
